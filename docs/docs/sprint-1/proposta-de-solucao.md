...
**Responsável:** Carlos Ícaro. **Etapa:** Sprint 1. **Versão:** inicial, para validação com a equipe e a Raiar.

Propomos desenvolver uma ferramenta de visão computacional para acompanhar a **distribuição das aves visíveis entre os níveis da estrutura Jump Start durante uma janela delimitada da recria**. A solução deverá transformar imagens em contagens e percentuais por nível, preservar seu histórico e permitir que a equipe técnica consulte a evolução por câmera, horário e idade do lote.

O primeiro produto mínimo viável (MVP) deverá demonstrar que essa medição é possível e útil com imagens reais da Raiar. A decisão de manejo continuará com a equipe responsável. A próxima decisão do grupo é confirmar o recorte de idade, a parte da estrutura observada e a viabilidade das imagens antes de iniciar o treinamento.
